 ...
 *
 * - --watch-debounce <ms>
 *   Debounce window for install trigger when artifact changes continuously. Default: 1500
 *
...
// Target editor page.
const TARGET_URL = "https://pro.lceda.cn/editor?cll=debug";
...
const WATCH_DEBOUNCE_MS = Number(CLI.watchDebounce || process.env.WATCH_DEBOUNCE_MS || 1500);
...
function createArtifactInfo(filePath) {
	...
	return {
		...
		// Use mtime as the primary build-change signal.
		key: `${filePath}|${Math.trunc(stat.mtimeMs)}|${stat.size}`
	};
}
...
function getCurrentPackageInfo() {
	...
	try {
		...
		const expectedArtifact = createArtifactInfo(expectedPath);
		if (expectedArtifact) {
...
async function closeBlockingModalIfPresent(page) {
	...
	if (escPressCount > 0) {
		console.log(`Action: pressed Esc ${escPressCount} time(s)`);
	}
}
...
async function runWatchMode() {
	...
	console.log(`Watch debounce: ${WATCH_DEBOUNCE_MS}ms`);
	...
	let debouncedArtifact = null;

	const triggerInstall = async (artifactInfo, reason) => {
		if (!artifactInfo || !fs.existsSync(artifactInfo.filePath)) {
			console.log("Install skipped: artifact file does not exist.");
			return;
		}
		...
		while (current) {
			if (!fs.existsSync(current.filePath)) {
				console.log(`Install skipped: artifact missing -> ${current.filePath}`);
			...
			console.log(
				`Install start [${currentReason}] => ${path.basename(current.filePath)} (mtime=${formatTime(current.mtimeMs)})`
			);
			try {
	...
	const scheduleDebouncedInstall = (artifactInfo, reason) => {
		debouncedArtifact = artifactInfo;
		if (debounceTimer) {
			clearTimeout(debounceTimer);
		}

		debounceTimer = setTimeout(() => {
			const latest = debouncedArtifact;
			debouncedArtifact = null;
			debounceTimer = null;

			if (!latest) {
				return;
			}
			if (latest.key === lastInstalledKey && !isInstalling) {
				return;
			}

			console.log(`Debounce flush => ${path.basename(latest.filePath)}`);
			void triggerInstall(latest, `${reason}-debounced`);
		}, WATCH_DEBOUNCE_MS);
	};
	...
	setInterval(() => {
		const artifact = getCurrentPackageInfo();
		if (!artifact) {
			return;
		}
		...
		scheduleDebouncedInstall(artifact, "artifact-changed");
	}, WATCH_INTERVAL_MS);
